      ...
      >
        <div class="card">
          <div class="card-image">
            <img :src="film.poster || '/placeholder-poster.png'" alt="Affiche du film" />
            <div class="card-rating" v-if="film.note?.mean">
              <span class="star">★</span> {{ parseFloat(film.note?.mean).toFixed(1) }}
            </div>
          </div>
          <div class="card-content">
            <h2>{{ film.title ?? 'Titre non renseigné' }}</h2>
            <p class="card-genre">{{ film.genres?.[0] ?? 'Genre inconnu' }}</p>
          </div>
        </div>
      </router-link>
    </div>
